...
impl<F: SemiImplicit> TimeEvolution for DiagRK4<F> {
        ...
        S: DataMut<Elem = Self::Scalar>,
    {
        ...
        Zip::from(&mut self.x)
            .and(&self.k1)
            .for_each(|x_, k1_| *x_ = *x_ + k1_.mul_real(dt_6));
        l.iterate(&mut self.x);

        Zip::from(&mut self.x)
            .and(&self.k2)
            .and(&self.k3)
            .for_each(|x_, &k2_, &k3_| *x_ = *x_ + (k2_ + k3_).mul_real(dt_3));
        l.iterate(&mut self.x);
